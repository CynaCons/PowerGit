...
#
# linuxdeploy-plugin-gtk also emits `export GDK_BACKEND=x11`. Keep X11 as
# its compatibility default, but never let the generated hook overwrite an
# explicit `GDK_BACKEND=wayland`; --fix rewrites it to
...
forced_x11_hook() {
  local hook="$1/apprun-hooks/linuxdeploy-plugin-gtk.sh"
  if [ -f "$hook" ] && grep -Eq '^[[:space:]]*export[[:space:]]+GDK_BACKEND=x11([[:space:]]*(#.*)?)?$' "$hook"; then
    echo "$hook"
  fi
  return 0
}
...
patch_gtk_backend_hook() {
  sed -i 's|^[[:space:]]*export[[:space:]]\+GDK_BACKEND=x11.*$|export GDK_BACKEND="${GDK_BACKEND:-x11}" # X11 default; allow explicit Wayland|' "$1"
}
...
self_test() {
  ...
  echo "== self-test: generated GTK backend hook =="
  mkdir -p "$w/apprun-hooks"
  local hook="$w/apprun-hooks/linuxdeploy-plugin-gtk.sh"
  printf '%s\n' 'export GDK_BACKEND=x11 # generated by linuxdeploy' >"$hook"
  if [ "$(forced_x11_hook "$w")" = "$hook" ]; then echo "  ok   forced X11 hook detected"
  else echo "  FAIL forced X11 hook was not detected" >&2; rc=1; fi
  patch_gtk_backend_hook "$hook"
  if [ -z "$(forced_x11_hook "$w")" ] && grep -Fq 'export GDK_BACKEND="${GDK_BACKEND:-x11}"' "$hook"; then
    echo "  ok   explicit backend is preserved after patch"
  else
    echo "  FAIL generated GTK hook was not patched" >&2; rc=1
  fi
...
scan() {
  problems=0; warnings=0; PROHIBITED=""; BROKEN=""; FORCED_X11=""
  local name reason paths p unresolved sev hook
  ...
  echo "== GTK backend policy =="
  hook=$(forced_x11_hook "$ROOT")
  if [ -n "$hook" ]; then
    FORCED_X11="$hook"
    if [ "$STRICT" = 1 ]; then sev=FAIL; problems=$((problems + 1)); else sev=WARN; warnings=$((warnings + 1)); fi
    echo "  $sev — generated GTK hook overrides explicit GDK_BACKEND (blocks native Wayland)"
  else
    echo "  explicit GDK_BACKEND is preserved"
  fi
...
if [ -n "$FORCED_X11" ]; then
  echo "  patch $FORCED_X11 to preserve an explicit GDK_BACKEND"
  patch_gtk_backend_hook "$FORCED_X11"
fi
