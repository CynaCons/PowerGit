...
export async function fetchWorkTreeDiff(path: string, staged = false): Promise<DiffDto> {
  const res = await fetch(`${ENGINE_URL}/diff/worktree?path=${encodeURIComponent(path)}&staged=${staged}`)
  return json<DiffDto>(res)
}
