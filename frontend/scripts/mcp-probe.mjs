...
const executable = process.argv[2] ?? process.env.POWERGIT_EXE ?? resolve(here, "../src-tauri/target/debug/powergit.exe");
...
try {
  ...
  const opened = payloadFrom(await request("tools/call", {
  ...
  const waited = payloadFrom(await request("tools/call", {
