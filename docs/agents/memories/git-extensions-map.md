...
- Refs on a row: GE's `Columns/MessageColumnProvider` draws every ref inline in the message cell (`RevisionGridRefRenderer.DrawRef` per `IGitRef`, the cell just clips) and `CommitInfo` lists them as "Contained in branches / tags" text (`RefsFormatter`, truncated at `MaximumDisplayedRefsIfLimited`). PowerGit folds by width into a "+n" chip that expands the row on demand (v0.18.3, `components/refChipsModel.ts`, one row at a time, the virtualizer measures it) and the Commit tab shows the same chips one size up (`CommitDetailView` Refs row). See [ref-chips.md](ref-chips.md).
...
- `RevisionGridControl.HighlightSelectedBranch` / `Command.ToggleHighlightSelectedBranch` (Ctrl+Shift+B, Alt+LButton in `OnGridViewMouseClick`; menu "Highlight selected branch (until refresh)" in `RevisionGridMenuCommands`; `RevisionGraphDrawStyle.HighlightSelected`) → PowerGit "Highlight ancestry (until refresh)" (v0.18.4): `graph/ancestry.ts markAncestry(rows, rootId)`, `useHistory.highlightRoot` (one per history, never the file history's from the main grid's), the row menu item `ctx-highlight-ancestry`, `browse.highlightAncestry` Ctrl+Shift+B, Alt+click, Esc with the grid focused, and the amber `GraphOptionsBar` with Exit. GE's FormFileHistory has no such item; the file history's grid still takes Alt+click and Esc. See [visual-direction.md](visual-direction.md).
- `RevisionGridControl` GoToParent (Ctrl+P: `ParentChildNavigationHistory` first, else `FirstParentId`), GoToChild (Ctrl+N: the memory first, else `GetRevisionChildren(...)[0]`, the topmost loaded child), SelectCurrentRevision (Ctrl+Shift+C → `CurrentCheckout`), NavigateBackward/Forward (Alt+←/→ over `NavigationHistory`) → PowerGit the compass (v0.18.12): `graph/graphNav.ts` (`parentsOf`, `firstChildOf`, `headSha`, `ParentChildMemory`, `NavHistory`), `hooks/useGraphNav.ts`, `components/GraphCompass.tsx` at the bottom-right of the grid body, `browse.goToParent` / `goToChild` / `goToHead` / `navigateBack` / `navigateForward`. GE has no buttons for these; PowerGit's pill is the owner's pick. See [graph-nav.md](graph-nav.md).
...
| `FormFormatPatch` (Commands → Format patch…: a revision range, an output directory, `GitModule.FormatPatch` = `git format-patch --find-renames --find-copies --break-rewrites -o <dir>`) | "Save as patch…" in the row menus (v0.18.6): one commit at a time, mailbox format (`-1 --stdout`) with GE's three flags and binary diffs on, written where the platform's Save dialog points under git's own `0001-<%f>.patch`; the pending rows get "Save changes as patch…" (`git diff [--cached] --binary`). No range and no directory: `GET /commits/{id}/patch` streams, the shell's `write_text_file` writes (see [save-patch.md](save-patch.md)) |
...
| — (PowerGit-only: Wrap lines, v0.18.8) | `FileViewer` has no wrap: long lines scroll sideways. PowerGit keeps that default and adds a remembered "Wrap lines" switch (`components/codeWrap.ts`, `pg.diffWrap`) in the diff pill (`DiffOptionsBar`) and the blob pane's own pill; presentation only, never a diff option. See [code-wrap.md](code-wrap.md) |
| `FilterInfo.ShowFilteredBranches` / `ShowCurrentBranchOnly` (the toolbar's branch filter text box, `GetBranchRevisionFilter` passing explicit revs to `git log`) | PowerGit's graph filter **mode** (v0.18.5): the funnel in the Repository tree header, checkboxes per ref with the checked-out branch locked, `GET /revisions?ref=…` with the names on `git log --stdin`; "current branch only" is the mode with nothing ticked. See [graph-ref-filter.md](graph-ref-filter.md) |
