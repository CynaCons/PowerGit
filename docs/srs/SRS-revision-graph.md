...
| SRS-GRAPH-007 | The lane layout shall match Git Extensions `RevisionGraph` on the golden fixtures under `frontend/src/graph/__fixtures__/ge` (node lane, occupied lanes and segment continuation per row); any fixture it does not match shall be listed with a reason in the parity test's `KNOWN_DIVERGENCES`. | GE is the behavioural reference; parity is proven, not claimed. | Test | `layout.ge-parity.test.ts`, `tools/ge-parity/` |
