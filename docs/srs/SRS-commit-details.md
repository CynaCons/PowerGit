...
| ID | Requirement | Rationale | Verification | Trace |
|---|---|---|---|---|
| SRS-DET-020 | Right-clicking a file in the Diff tab's file list shall offer a reset of that file, plus Open with difftool, Copy path and (v0.16.0) View file history. | GE's `FileStatusList` context menu, in the surface where the user is already reading the change. | Test | `DiffContextMenus.tsx`, `browse-reset.spec.ts`, `file-history.spec.ts` |
...
Owner: "In main view, in the file tree, right click a file and show the file history. Here again, we have to be functionally equivalent to GE." GE's `FormFileHistory`.

| ID | Requirement | Rationale | Verification | Trace |
|---|---|---|---|---|
| SRS-DET-030 | A file (or folder) in the File Tree tab, in the Diff tab's file list and in the commit dialog's lists shall offer "View file history" on its context menu, and Ctrl+Shift+H shall open it for the file the panel has selected. | GE `FileStatusList` "File history" (`ShowHistory` hotkey) on every file surface. | Test | `CommitFileTree.tsx`, `DiffContextMenus.tsx`, `commitFileMenuModel.ts`, `file-history.spec.ts` |
| SRS-DET-031 | The file history shall show the revision grid limited to the commits that touched the path, with the same graph, columns, row menu and ref menus as the main history, under a header naming the path (and the file's name at the selected commit when it differs) with Follow renames, Exact renames only, Full history and Simplify merges toggles that reload the list. | GE `FormFileHistory` window: `RevisionGrid` + its menus; `SetTitle` shows the alternative name. | Test | `FileHistoryView.tsx`, `fileHistoryModel.ts`, `file-history.spec.ts` |
| SRS-DET-032 | Under the grid, Commit, Diff (that file at the selected commit against its parent, the Diff tab first) and View (the file's content at that commit) shall be offered; a pending-change row shall offer Diff only and a folder no View. Blame is reserved until a blame view exists. | GE `UpdateSelectedFileViewers`: the tab strip changes with the row. | Test | `fileHistoryTabs`, `fileHistoryModel.test.ts`, `file-history.spec.ts` |
| SRS-DET-033 | Escape or the header's close button shall return to the full history with its rows, selection and scroll intent intact, without refetching the graph. | GE closes the window on Escape from any of its viewers; the main history was never unloaded. | Test | `useFileHistory.ts`, `file-history.spec.ts` |
