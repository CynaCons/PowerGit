...
### v0.16.0 — Commit window you can size and close, GE file menus, copyable diffs, a console you can read (current) (ACTIVE)
...
- [ ] Owner bug (2026-09-11): "accessing a file in the file tree when selecting the working directory pseudo commit doesn't load. It should load the latest commit on that branch on which the working directory is based." The File Tree tab for the artificial Working directory (and Index) row: list HEAD's tree (the commit the working directory is based on) and serve the file's content from the working tree for the Working directory row (from the index for the Index row), so a modified file shows its current text; the same in the file history view when the pending row is selected. Symptom e2e: select the Working directory row, open File Tree, click a modified file, its current content is shown. [agent: claude]
...
- [ ] Codex Sol code review of everything since v0.15.7 (the six features above): correctness bugs, GE-parity gaps and risky spots, file:line, nothing edited; findings triaged into fixes or recorded before the release. [agent: claude]
