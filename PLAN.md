...
### v0.15.6 — Ubuntu freeze taskforce — the main loop is alive, instrument what paints and let the owner drive recovery (current) (ACTIVE)
...
- [ ] Case file corrected: docs/ubuntu-freeze.md and docs/agents/memories/diagnostics.md no longer claim the main loop is parked when the evidence shows it dispatching IPC; the inspector argument restated as a shared-presentation-path symptom. [agent: claude]
- [ ] Shell probes that work when the main loop is alive but nothing paints: GDK paint age next to frame age in the watchdog observation, shell.txt and a periodic engine.log line; main-loop liveness round-trip from a dedicated thread, recorded on disk for freeze-dump.sh. [agent: claude]
- [ ] Recovery-experiment ladder the owner drives during a freeze, each step distinct and numbered in engine.log (queue draw, thaw updates, hide/show, 1px resize, webview reload, new window), reachable from the page and from a native fallback. [agent: claude]
- [ ] Owner ticks: next freeze reported with the engine.log recovery numbers and, if possible, a freeze-dump capture. [agent: claude]
- [ ] Native Wayland is the default on a Wayland session (POWERGIT_X11=1 keeps XWayland), and POWERGIT_NO_FRAME_SYNC=1 disables GTK's X11 frame sync: the two switches that discriminate H1 (GdkFrameClock frozen waiting for _NET_WM_FRAME_DRAWN under XWayland). Linux compile gate run in WSL (cargo check + cargo test --lib). [agent: claude]
