...
- [ ] Commit window: resizable by dragging its edges and corners, size remembered across opens; a close X in the top-right corner of the dialog (Escape and the Cancel button keep working). Symptom test: open the commit dialog, drag the corner, reopen, the size sticks; the X closes it. [agent: claude]
- [ ] Commit window as a separate native window (stretch goal, only if it does not endanger the resizable dialog): a second Tauri WebviewWindow hosting the commit view against the same engine session, with the dialog as the fallback in the browser and on platforms where it misbehaves. [agent: claude]
- [ ] Commit file lists: right-click menu on staged and unstaged files with Git Extensions' FileStatusList item set and behaviour: stage/unstage, reset file changes, delete, open, open with, show in folder, copy full path, edit file, view file history, add to .gitignore, add to .git/info/exclude, skip-worktree and assume-unchanged toggles, interactive stage (selected lines). Every item works against the engine (new endpoints where missing, with xunit tests) or is hidden, never a dead item. Read ../gitextensions-ref FileStatusList and FormCommit for the reference list and record the mapping in docs/agents/context/git-extensions-map.md. [agent: claude]
- [ ] Diff views (commit dialog and Browse diff tab): plain text selection with the mouse for copy-paste works again alongside line selection for reset. Dragging across text selects text; click / Ctrl+click / Shift+click on a line still selects lines; Ctrl+C copies the text selection. Symptom test: select two words in a diff line, Ctrl+C, the clipboard holds them. [agent: claude]
- [ ] No false 'git failed' for a new file: the diff of an untracked file runs git diff --no-index, which exits 1 by design whenever the file has content, and the console model marks every non-zero exit as a failure (gitLogModel.ts failed/notableFailure). The engine's command log gains the caller's verdict (expected exit codes) so the console only flags real failures; unit test on the model, xunit on the log entry. [agent: claude]
- [ ] Git console you can read: what the user did (push, pull, fetch, commit, merge, rebase, stash) stands out from the background probes (status, log, diff, ls-files, rev-parse) which are collapsed or hidden by default; a filter toggle shows everything; a failed user action is pinned at the top until dismissed; the entry for a push shows its full output. Symptom test: after a push, the console shows the push entry within the first three rows without scrolling. [agent: claude]
