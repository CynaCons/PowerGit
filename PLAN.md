...
- [ ] Owner request (2026-09-05): "On Windows and Linux, can we merge the appbar at the top with the window frame itself? integrating the three system-window buttons top right of the appbar with same functionality?" Frameless window (decorations off) with the command bar as the title bar: drag region on the bar's empty space, double-click toggles maximize, Minimize / Maximize-Restore / Close controls at the right end in the platform's glyph style with the same behaviour as the native buttons (close asks the shell to close so the engine exits with it), maximize state reflected in the glyph, controls hidden in the browser/e2e. Tauri capabilities for window control; works on Windows 11 and Ubuntu (X11 and Wayland) with edge resizing intact; captures on both. Owner ticks. [agent: claude]
