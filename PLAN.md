...
- [ ] Selected commit vanishes from the graph: the opaque .grid-row.selected background (added in v0.12.1, z-index above the canvas) covers the canvas node and lane lines of the selected row on every platform. Keep the selection band on the canvas and restrict the DOM background to the text cells; prove it with a pixel-sampling e2e test that clicks a row and requires the node/lane colours in the graph column of the selected row, in light and dark themes and at 100%/150% zoom, before the CSS change (red) and after (green). [agent: claude]
- [ ] Immediate selection: clicking a row must apply the highlight in the same frame; commit details, file list and diff load asynchronously afterwards without blocking the click. Measure click-to-highlight with a Playwright timing probe (long tasks + rAF), find the blocking work (unmemoized re-render of the grid/left panel/bottom panel, synchronous effects), fix it, and assert a click-to-highlight budget in perf.spec. [agent: claude]
